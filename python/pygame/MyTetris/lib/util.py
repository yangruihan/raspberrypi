...
def file_path(filename = None):
    ...
    if os.path.abspath(file_path):
        ...
        raise ValueError, "Can't open file '%s'." % file_path

def myprint(screen, string, pos, size='s', color = (255, 255, 255)):
    """
    Print text to game display
    :param screen: display screen
    :param string: the text want to display
    :param pos: the position of text
    :param size: size of text(can be l, m ,s)
    :param color: the color of text
    """
    font_style = _FONTS[size].render(str(string), True, color)
    screen.blit(font_style, pos)
